import { Alert, AlertIcon, Box, Button, HStack, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Stack, Text, Textarea, useDisclosure } from "@chakra-ui/react";
...
import { TSimpananRequest, useCreateSimpanan, useSimpananSebelumnya } from "@/services/api/commands/simpanan.command";
...
const ModalCreateDebit = forwardRef<
  ...
  any>((_, ref) => {
    ...
    const handleChangeNominal = (nominal: number) => {
      ...
      form.setValue('nominal', isNaN(nominal) ? '' : tempNominal.toLocaleString('id-ID'))
      ...
      form.setValue('nominalWajib', isNaN(wajib) ? '' : parseInt(`${wajib}`.replace(/\D/g, ''), 10).toLocaleString('id-ID'))
      form.setValue('nominalKhusus', isNaN(khusus) ? '' : parseInt(`${khusus}`.replace(/\D/g, ''), 10).toLocaleString('id-ID'))
      form.setValue('nominalSukarela', isNaN(sukarela) ? '' : parseInt(`${sukarela}`.replace(/\D/g, ''), 10).toLocaleString('id-ID'))
    }
    const watchFields = form.watch(['nominalWajib', 'nominalKhusus', 'nominalSukarela'])
            ...
            <ModalBody>
              <Stack spacing={3}>
                ...
                <InputText
                  ...
                  onChange={(e) => {
                    const nominal = parseInt(e.currentTarget.value.replace(/\D/g, ''), 10);
                ...
                <Box >
                  ...
                  <Textarea placeholder="masukkan keterangan simpanan" h="150px" onChange={(e) => form.setValue('catatan', e.target.value)} />
                </Box>
              </Stack>
            </ModalBody>
          }
